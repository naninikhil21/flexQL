...
static const int INSERT_BATCH_SIZE = 1000000; // larger bulk command chunk for minimal request overhead
...
static bool run_insert_benchmark(FlexQL *db, long long target_rows) {
    ...
    while (inserted < target_rows) {
        long long batch_rows = static_cast<long long>(INSERT_BATCH_SIZE);
        if (batch_rows > (target_rows - inserted)) {
            batch_rows = target_rows - inserted;
        }

        std::string sql;
        sql.reserve(96);
        sql += "BULK_INSERT BIG_USERS ";
        append_i64(sql, inserted + 1);
        sql += " ";
        append_i64(sql, batch_rows);
        sql += ";";
