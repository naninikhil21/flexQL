...
int flexql_exec(
    ...
    if (!db) return FLEXQL_ERROR;
    if (!sql) return FLEXQL_ERROR;

    // Ensure SQL ends with ';' to prevent server-side hang (bug 3.4)
    std::string sql_str(sql);
    if (!sql_str.empty() && sql_str.back() != ';') {
        sql_str.push_back(';');
    }

    if (send(db->sock, sql_str.c_str(), sql_str.size(), MSG_NOSIGNAL) < 0) {
        if (errmsg) {
        ...
        while ((newlinePos = pending.find('\n')) != std::string::npos) {
            ...
            if (callback && line.rfind("ROW ", 0) == 0) {
                ...
                if (parse_row_payload(rowValue, values, columnNames)) {
                    ...
                    col[0] = (char*)"row";
                    int cb_ret = callback(arg, 1, argv, col);
                    if (cb_ret != 0) {
                        done = true;
                        break;
                    }
